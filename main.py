...
from ImageNetDataset import INDEX_START_POINT, CLASS_RANGE    # IMAGENET 2012   # 모델에는 따로 선언해줌
...
# hands-on machine learning 에서는 r=2 a = 0.00002, b = 0.75, k =1 이라고 되어있음
# 문서에는 5, 1e-4, 0.75 2
LRN_INFO = (5, 1e-04, 0.75, 2) # radius, alpha, beta, bias   
INPUT_IMAGE_SIZE = 227 #WIDTH, HEIGHT    # cropped 256x256 images
WEIGHT_DECAY = 5e-4
...
def _parse_function(example_proto):
    ...
    label = tf.cast(tf.subtract(label,INDEX_START_POINT), tf.int32)
    return image, label
...
if __name__ == "__main__":
    ...
    parser.add_argument('--traindir', type=str, default=os.path.join(root_dir, f"tfrecord_train_class{CLASS_RANGE}"), required=True) 
    parser.add_argument('--valdir', type=str, default= os.path.join(root_dir, f"tfrecord_val_class{CLASS_RANGE}"), required=True) 
    
    args = parser.parse_args
    
    TRAIN_DATASET = args.traindir
    TEST_DATASET = args.valdir
    ...
    train_dirs = os.listdir(TRAIN_DATASET)
    test_dirs = os.listdir(TEST_DATASET)
    
    for train_dir in train_dirs:
        dir_path = os.path.join(TRAIN_DATASET, train_dir)
        a =tf.data.Dataset.list_files(os.path.join(dir_path, '*.tfrecord'))
        train_tfrecord_list.extend(a)
    
    for test_dir in test_dirs:
        dir_path = os.path.join(TEST_DATASET, test_dir)
    ...
    _model = model.mAlexNet(LRN_INFO, CLASS_RANGE)
